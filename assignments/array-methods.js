...
runners.forEach(function(data){
  return fullNames.push(`${data.first_name} ${data.last_name}`)
});
...
const firstNamesAllCaps = runners.map(function(data){
...
const runnersLargeSizeShirt = runners.filter(function(){
...
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
  ...
  const ticketPriceTotal = runners.reduce(function(add, data){
    return add + data.donation;
  }, 0);
